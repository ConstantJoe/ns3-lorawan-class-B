                                       ...
                                       Ptr<EnergySource> source) const
{
  ...
  if (deviceName.compare ("ns3::LoRaWANNetDevice") != 0 && 0 != device->GetInstanceTypeId().GetParent ().GetName ().compare ("ns3::LoRaWANNetDevice"))
   {
  ...
  Ptr<LoRaWANNetDevice> LoRaDevice = DynamicCast<LoRaWANNetDevice>(device);
 ...
 if (deviceName.compare ("ns3::LoRaNetDevice") != 0 && 0 != device->GetInstanceTypeId().GetParent ().GetName ().compare ("ns3::LoRaNetDevice"))
  ...
  LoRaPhy -> TraceConnectWithoutContext ("StateValue",MakeCallback(&LoRaRadioEnergyModel::ChangeLoRaState, model)); //TODO: this is the important linker, this has to be updated
